 ...
 * @class Fixed
 */
class FixedDetail extends Models\SalaryDetail
{
    /**
    ...
    protected static $jqxGridColumns = [
        ...
        [
            'text' => 'L.khoán (đơn giá)',
            'datafield' => ['salary_fixed', [
                'type' => 'number'
            ]],
            'width' => 128,
            'cellsalign' => 'right',
    ...
    {
        return $this->belongsTo(Fixed::class, $this->columnName('salary_id'));
    }
   ...
   //.end
    ...
    public function assignSalaryAndTimeSheetInfo(Models\SalaryConfig $salaryConfig, Models\Account $account, array $salaryTSInfo, array $options = [])
    {
        ...
        // +++ % Cong doan phi
        $perEmplPayUnionCost = $salaryConfig->colVal('per_employees_pay_union_cost');
        ...
        $slrdInfo = [
            ...
            ($colPrefix . ($col = 'salary_fixed_final')) => ($salaryFixedFinal 
                = ($slrFixedUnitPriceDay * $this->colVal('time_worktime'))
            ),
            ...
            ($colPrefix . ($col = 'salary_deduction_others')) => ($salaryDeductionOthers
                = (1 * $slrCnfSlrFixed['deduction_others'])
            ),
            // +++ Con lai...
     ...
     * @return void
     */
    public function jqxFetchRecordList(Fixed $salaryModel, array $data, &$qB = null, &$totalRowsQB = null)
    {
        // Define vars
        // +++
        //Roles
    ...
    public function fetchReportDataByTime(array $options, &$qB = null, &$totalRowsQB = null)
    {
        ...
        $options = array_replace($options, [
            'salary_type' => Models\Salary::TYPE_80,
        ]);
        //
